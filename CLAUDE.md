...
- `service/DuelService|DuelBattleService|DuelSettlementService|DuelImpeachmentService|DuelInviteService` — 死斗生命周期（创建/加入/申请审批/移除/让渡/影子计划/开赛）、打卡凭证与审核、幂等结算、弹劾组长（票数严格过半即发起人接任，24h 未过半判负 + 判负后 24h 冷却，扫期任务 + 惰性兜底）、邀请海报（每成员每局一个固定码 + 海报复用，`app.invite.base-url` 决定二维码内容；仅招募中有效，组长码审批制也免审直入、成员码审批制走申请并留痕 `duel_join_request.inviter_id`）
...
- `service/StorageService` — 文件存储抽象，双实现按 `app.storage.type` 条件装配（默认 `local` 本地磁盘；`oss`=阿里云 OSS，凭证走 `ALIYUN_*` 环境变量）。`storeImage(MultipartFile)` 返回原图+预览图双 URL（服务端 `ImageUtils` 压缩：≤750px/JPEG 0.7/白底），`storeImage(byte[], ext)` 存服务端生成的图（邀请海报，无预览图），`readAllBytes` 供 AI 读图。`/uploads/**` 静态映射的路径必须 `normalize()`（相对路径带 `/./` 的 file URI 会 404）
- `service/EvidenceAiPreCheckService` — 凭证 AI 预审扩展点（预留方法）：图片存储成功即预审，结论缓存 Redis（TTL 7 天），死斗凭证提交时消费；`app.ai.vision.*` 开关关闭时全链路空操作。视觉服务 GLM-4V 走 OpenAI 兼容端点，RestClient + 虚拟线程限时
